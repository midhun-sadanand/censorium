...
class PlateEvaluator:
    """
    Evaluator for license plate detection performance
    """
    
    def __init__(self, detector: LicensePlateDetector, iou_threshold: float = 0.5):
        """
...
def create_sample_dataset(output_dir: str, num_images: int = 50):
    ...
    for i in range(num_images):
        ...
        # Generate random license plate bounding boxes
        num_plates = np.random.randint(0, 3)
        bboxes = []
...
def plot_results(results: Dict, output_path: str):
    ...
    axes[0].set_title('License Plate Detection Metrics')
...
def plot_confidence_threshold_analysis(threshold_results: Dict, output_path: str):
    """
    Plot analysis of performance vs confidence threshold
    
    Args:
        threshold_results: Dictionary mapping thresholds to metrics
        output_path: Path to save plot
    """
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate license plate detection performance')
    ...
    parser.add_argument('--output', type=str, default='plate_eval_results.json', help='Output JSON file')
    parser.add_argument('--plot', type=str, default='plate_eval_plot.png', help='Output plot file')
    parser.add_argument('--create-sample', action='store_true', help='Create sample dataset')
    parser.add_argument('--sample-dir', type=str, default='sample_plate_dataset', help='Sample dataset directory')
    parser.add_argument('--threshold-analysis', action='store_true', help='Run confidence threshold analysis')
    ...
    # Initialize evaluator
    evaluator = PlateEvaluator(detector, iou_threshold=0.5)
    
    # Run threshold analysis if requested
    if args.threshold_analysis:
        print("Running confidence threshold analysis...")
        threshold_results = evaluator.evaluate_confidence_thresholds(
            args.dataset,
            args.annotations
        )
        
        # Save threshold results
        threshold_output = args.output.replace('.json', '_thresholds.json')
        with open(threshold_output, 'w') as f:
            json.dump(threshold_results, f, indent=2)
        print(f"Threshold results saved to {threshold_output}")
        
        # Plot threshold analysis
        threshold_plot = args.plot.replace('.png', '_thresholds.png')
        plot_confidence_threshold_analysis(threshold_results, threshold_plot)
        return
    ...
    # Print results
    print("\n" + "="*50)
    print("LICENSE PLATE DETECTION EVALUATION RESULTS")
    print("="*50)
